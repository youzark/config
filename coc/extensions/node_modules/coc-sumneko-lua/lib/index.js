  ...
  function patch(fs4) {
    ...
    function readdir(path4, options, cb) {
      ...
      function go$readdir(path5, options2, cb2, startTime) {
        return fs$readdir(path5, options2, function(err, files) {
          if (err && (err.code === "EMFILE" || err.code === "ENFILE"))
            enqueue([go$readdir, [path5, options2, cb2], err, startTime || Date.now(), Date.now()]);
          else {
            if (files && files.sort)
              files.sort();
            if (typeof cb2 === "function")
...
var URL = import_whatwg_url.default.URL;
...
var fs = __toModule(require_lib2());
...
function showTooltip(ctx) {
  return async () => {
    import_coc2.window.showNotification({content: ctx.barTooltip, timeout: 5e3});
  };
}
...
var fs2 = __toModule(require_lib2());
...
var Config = class {
  ...
  get inlayHints() {
    const hasVirtualText = import_coc3.workspace.isNvim && import_coc3.workspace.nvim.hasFunction("nvim_buf_set_virtual_text");
    return {
      enable: hasVirtualText && import_coc3.workspace.getConfiguration("Lua.hint").get("enable", false),
      typeHintsPrefix: this.cfg.get("inlayHints.typeHintsPrefix", "\xAB "),
      paramHintsPrefix: this.cfg.get("inlayHints.paramHintsPrefix", "\xBB "),
      trimSemicolon: this.cfg.get("inlayHints.trimSemicolon", true),
      refreshOnInsertMode: hasVirtualText && this.cfg.get("inlayHints.refreshOnInsertMode", false)
    };
  }
};
...
var inlayHintsNS = import_coc4.workspace.createNameSpace("lua-inlay-hint");
var HintKind;
(function(HintKind2) {
  HintKind2[HintKind2["TypeHint"] = 1] = "TypeHint";
  HintKind2[HintKind2["ParamHint"] = 2] = "ParamHint";
})(HintKind || (HintKind = {}));
var InlayHintsController = class {
  ...
  async activate() {
    ...
    const currentDoc = await import_coc4.workspace.document;
    if (isLuaDocument(currentDoc.textDocument)) {
      this.fetchAndRenderHints(currentDoc);
    }
  }
  async hintHandler(params) {
    const hints = [];
    for (const edit of params.edits) {
      hints.push({
        kind: (edit.text ? edit.text : edit.newText).startsWith(":") ? 1 : 2,
        text: edit.text ? edit.text : edit.newText,
        pos: edit.pos ? edit.pos : edit.range.start
      });
    }
    await this.renderHints(import_coc4.workspace.getDocument(params.uri), hints);
  }
  async fetchAndRenderHints(doc) {
    if (!(this.inlayHintsEnabled && doc && isLuaDocument(doc.textDocument)))
      return;
    try {
      ...
      this.ctx.client.sendNotification("$/didChangeVisibleRanges", {
        uri: doc.uri,
        ranges: [
          import_coc4.Range.create({line: 0, character: 0}, {line: lastLineNumber, character: doc.getline(lastLineNumber).length})
        ]
      });
    }
  }
  async fetchHints(doc) {
    return this.ctx.client.sendRequest("$/requestHint", {
      textDocument: doc.textDocument,
      range: import_coc4.Range.create(0, 0, doc.lineCount, 0)
    });
  }
...
var Ctx2 = class {
  ...
  resolveBin() {
    ...
    const bin = import_path3.default.join(serverDir, "bin", platform2 === "win32" ? "lua-language-server.exe" : "lua-language-server");
    ...
    const args = ["-E", import_path3.default.join(serverDir, "bin", "main.lua"), `--locale=${this.config.locale}`].concat(import_coc5.workspace.getConfiguration("Lua").get("misc.parameters"));
    ...
    if (this.config.prompt) {
      ...
      if (ret === 0) {
        ...
        try {
          ...
          console.error(e);
          import_coc5.window.showMessage("Upgrade server failed", "error");
          return;
        }
        await this.client.stop();
        this.client.start();
      } else {
        import_coc5.window.showMessage(`You can run ':CocCommand sumneko-lua.install' to upgrade server manually`);
      }
    } else {
      import_coc5.window.showMessage(`${msg} Run :CocCommand sumneko-lua.install to upgrade`);
    }
  }
  createClient() {
      ...
      middleware: {
        workspace: {
          configuration: async (params, token, next) => {
            ...
            const types = `${import_path3.default.dirname(import_path3.default.dirname(__filename))}/nvim_lua_types`;
  ...
  activateCommand() {
    this.client.onNotification("$/command", (params) => {
      ...
      for (const data of params.data) {
        const config = import_coc5.workspace.getConfiguration(void 0, data.uri);
        if (data.action == "add") {
  ...
  async activateInlayHints() {
    await import_coc5.workspace.nvim.command("hi default link CocLuaTypeHint  CocCodeLens");
    await import_coc5.workspace.nvim.command("hi default link CocLuaParamHint CocCodeLens");
...
async function activate(context) {
  ...
  if (!bin) {
    ...
    if (ctx.config.prompt) {
      ret = await import_coc6.window.showQuickpick(["Yes", "Cancel"], "Sumneko lua language server is not found, install now?");
    }
    if (ret === 0) {
      try {
        ...
        console.error(e);
        import_coc6.window.showMessage("Download Sumneko lua language server failed", "error");
        return;
      }
    } else {
      import_coc6.window.showMessage(`You can run ':CocCommand sumneko-lua.install' to install server manually`);
      return;
    }
  }
  ...
  ctx.registerCommand("restart", (ctx2) => {
    return async () => {
      import_coc6.window.showMessage(`Reloading sumneko lua-language-server...`);
      ...
      import_coc6.window.showMessage(`Reloaded sumneko lua-language-server`);
    };
  });
